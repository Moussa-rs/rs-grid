...
export default {
  ...
  created() {
    ...
    this.setBoundedHandler = (isBounded) => {
      if (this.isBounded === null) {
        this.bounded = isBounded
      }
    }

    this.setTransformScaleHandler = (transformScale) => {
    ...
    this.eventBus.$on('setBounded', this.setBoundedHandler)
    this.eventBus.$on('setTransformScale', this.setTransformScaleHandler)
  ...
  beforeUnmount() {
    ...
    this.eventBus.$off('setBounded', this.setBoundedHandler)
    this.eventBus.$off('setTransformScale', this.setTransformScaleHandler)
  ...
  mounted() {
    if (this.layoutInstance.responsive && this.layoutInstance.lastBreakpoint) {
      this.cols = getColsFromBreakpoint(
        this.layoutInstance.lastBreakpoint,
        this.layoutInstance.cols
      )
    } else {
      this.cols = this.layoutInstance.colNum
    }
    this.rowHeight = this.layoutInstance.rowHeight
    this.containerWidth = this.layoutInstance.width !== null ? this.layoutInstance.width : 100
    this.margin = this.layoutInstance.margin !== undefined ? this.layoutInstance.margin : [10, 10]
    this.maxRows = this.layoutInstance.maxRows

    if (this.isDraggable === null) {
      this.draggable = this.layoutInstance.isDraggable
    } else {
      this.draggable = this.isDraggable
    }
    if (this.isResizable === null) {
      this.resizable = this.layoutInstance.isResizable
    } else {
      this.resizable = this.isResizable
    }
    if (this.isBounded === null) {
      this.bounded = this.layoutInstance.isBounded
    } else {
      this.bounded = this.isBounded
    }
    this.transformScale = this.layoutInstance.transformScale
    this.useCssTransforms = this.layoutInstance.useCssTransforms
    this.useStyleCursor = this.layoutInstance.useStyleCursor
    this.createStyle()
  },
  watch: {
    isDraggable() {
      ...
      switch (event.type) {
        ...
        case 'dragmove': {
          ...
          if (this.bounded) {
            ...
            const rightBoundary =
              this.containerWidth - this.calcGridItemWHPx(this.w, colWidth, this.margin[0])
